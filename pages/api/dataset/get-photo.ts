...
export default async function handler(
  ...
  try {
    ...
    const isExistedData = await FIND_LOCKED_PHOTO_TO_USER(id);
    ...
    if (!isExistedData) {
      const data = await FIND_ONE_PHOTO(id);
      console.log({ data });

      if (data) {
        const update_lock = await LOCK_PHOTO_TO_USER(data._id, id);
